...
if __name__ == "__main__":
    ...
    obs_base_path = 'huawei-henan:openloong-zhengzhou-apps-private/data-collector-svc/align'
    rclone_config = '/root/.config/rclone/rclone.conf'
    local_base_path = '/mnt/sdc/align_leju_zz'

    excel_path = os.environ.get("BATCH_XLSX", "/root/convert2lerobotv30_new/数据转换第4批次20260624.xlsx")
    sheet_name = os.environ.get("BATCH_SHEET", "郑州平台")
    robot_type = "乐聚KUAVO"
    output_base_path = '/mnt/sdc/乐聚KUAVO_zhengzhou_v30_limited60'


    # 转换脚本路径
    CONVERT_SCRIPT = "/root/convert2lerobotv30_new/align_scripts/leju_align2lerobot_v30_no_norm.py"
    ...
    # 创建状态日志文件（支持resume模式）
    log_file_path = '/root/convert2lerobotv30_new/convert_all_leju_zhengzhou_status.txt'
    ...
    try:
        ...
        for i in range(total_tasks):
            ...
            if not logger.should_convert(task_id):
                ...
                try:
                    cmd = f'{shlex.quote(sys.executable)} {CONVERT_SCRIPT} --input {local_base_path}/{task_id} --output {output_base_path}/{task_id} --task {shlex.quote(processed_text_en)} --workers 20'
