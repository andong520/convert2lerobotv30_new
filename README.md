...
│   └── run_all.sh          #   串行队列（批量）
├── pipelines_noupload/     # 不上传版（同上但跳过上传），全是瘦封装
├── analyze_lerobot_data.py # 数据集统计工具（可选）
├── 全机型数据维度config说明.pdf   # 维度规范（真值来源）
└── 数据转换第三批次20260513 .xlsx  # 任务清单（驱动读取）
```
...
```
四阶段：H5→v30 → v30→v21 → 校验 → 上传。**校验不过自动停、不上传**。

### 5.2 不上传
```bash
bash pipelines_noupload/run_leju.sh      # 或等价: UPLOAD=0 bash pipelines/run_leju.sh
```

### 5.3 串行队列
```bash
bash pipelines/run_all.sh                 # 全部(带上传)
bash pipelines/run_all.sh leju gr2 R1     # 只跑指定几个
bash pipelines_noupload/run_all.sh        # 全部但不传
```
...
### 6.4 每个驱动的固定配置（写死在脚本 `__main__` 里）
所有驱动公共：`rclone_config=/root/.config/rclone/rclone.conf`、`excel_path=/root/convert2lerobotv30_new/数据转换第三批次20260513 .xlsx`、`MAX_DOWNLOAD_PER_ID=60`、`MAX_COUNT=300000`(子目录上限,实际不限)。各自不同的：
...
| 是否上传 | 加 `UPLOAD=0` 或用 pipelines_noupload/ | 环境变量 |
