#!/bin/bash
# qinglongros1 全流程: H5->v30 -> v30->v21 -> 校验 -> 上传   (60条限制保留, 由 convert_all 控制)
set -o pipefail
[ "${1:-}" = "upload" ] && UPLOAD=1   # 开启上传(默认不传); 等价 UPLOAD=1 bash 本脚本
PY=/root/miniconda3/bin/python3
BASE=/root/convert2lerobotv30_new
# ===== per-robot 配置 (按需改) =====
DRIVER="$BASE/shanghai/convert_all_qinglongros1.py"
ALIGN_CACHE="/mnt/sdc/align_qinglong"
STATUS="/root/convert2lerobotv30_new/convert_all_qinglongros1_shanghai_status.txt"
V30="/mnt/sdc/qinglongros1_shanghai_v30_limited60"
V21="/mnt/sdc/qinglongros1_shanghai_v21_limited60"
ROBOT="QinLongROS1"          # v21 robot-layer 子目录 = robot_type
STATE_DIM=16
...
# ===================================
LOG=/mnt/sdc/pipeline_qinglongros1.log
exec > >(tee -a "$LOG") 2>&1
echo "[$(date)] ===== qinglongros1: 全流程开始 ====="
mkdir -p "$ALIGN_CACHE"
rm -f "$STATUS"
...
echo "[$(date)] ===== qinglongros1: ALL DONE ====="
