#!/usr/bin/env python3
"""
灵龙 LINGLONG-H 人形机器人数据转换脚本 - 转换为 LeRobot v3.0 格式
【no_norm 版本：所有维度保留原始物理量，不做任何归一化】

厂商: 人形机器人（上海）有限公司, model=LINGLONG-H, metadata.ver=2.1.0
适用于已对齐的 h5 文件 (*_align.h5)。

注: 灵龙 h5 中 effector 前 2 维原始数据本身已是 [0, 1] 归一化的夹爪开度，
    因此标准版与本 no_norm 版数据层面行为完全等价（仅命名约定区分）。

数据结构 (基于 metadata.json + 实际 h5 dataset shape):
    State / Action: 24 维
        - arm:       14 维 (left_arm 7 + right_arm 7), 单位 rad
                     [J_arm_l_01..07, J_arm_r_01..07]
        - effector:   4 维, h5 实际 shape (N, 4)
                     [0] left_gripper   ∈ [0, 1]  ← 已归一化的左爪开度
                     [1] right_gripper  ∈ [0, 1]  ← 已归一化的右爪开度
                     [2] effector_reserved_2  (h5 中全 0，占位通道)
                     [3] effector_reserved_3  (h5 中全 0，占位通道)
        - head:       2 维 [J_head_yaw, J_head_pitch], 单位 rad
        - waist:      4 维, h5 实际 shape (N, 4) (metadata 仅列出 3 个 joint_names)
                     [J_waist_pitch, J_waist_roll, J_waist_yaw, J_waist_reserved_3]

    备注:
        - h5 中 joints/{state,action}/end 形状 (N, 6) / (N, 8)，是末端 link 的
          xyz 与 orientation，与 LeRobot 1D state vector 不兼容，舍弃。
        - metadata 中虽列出 chassis 字段，但本 h5 中实际无 chassis dataset，忽略。
        - effector 前 2 维原始数据已经是 [0, 1] 归一化的夹爪开度，因此本脚本
          不再额外归一化。本文件与 linglong_align2lerobot_v30_no_norm.py
          在数据层面行为完全等价。

    相机:
        - head:       原 720x960 RGB, resize 到 640x480
        - hand_left:  原 540x960 RGB, resize 到 640x480
        - hand_right: 原 540x960 RGB, resize 到 640x480

Usage:
    python linglong_align2lerobot_v30.py \\
        --input ./raw_data \\
        --output ./lerobot_dataset \\
        --task "Sort red and green peppers" \\
        --fps 30 --workers 4
"""
...
LINGLONG_CONFIG = {
    "robot_type": "LinglongH",
    "arm_dim": 14,       # 左臂7 + 右臂7
    "effector_dim": 4,   # h5 实际 4 维 (前2: 左右夹爪[0,1]; 后2: 占位通道)
    "head_dim": 2,       # head_yaw + head_pitch
    "waist_dim": 4,      # h5 实际 4 维 (metadata 仅列出 3 个名字)
    "cameras": ["head", "hand_left", "hand_right"],
    "target_image_size": (640, 480),  # (width, height) - 统一 resize 宽640高480
}

# 关节名顺序须与 load_linglong_h5 中 concat 顺序一致
MOTOR_NAMES = [
    # arm 14
    ...
    # effector 4 (前2为实际夹爪, 后2为占位通道)
    "left_gripper", "right_gripper",
    "effector_reserved_2", "effector_reserved_3",
    # head 2
    "J_head_yaw", "J_head_pitch",
    # waist 4 (metadata 列 3 个 + 1 个占位)
    "J_waist_pitch", "J_waist_roll", "J_waist_yaw", "J_waist_reserved_3",
]

STATE_DIM = (
    LINGLONG_CONFIG["arm_dim"]
    + LINGLONG_CONFIG["effector_dim"]
    + LINGLONG_CONFIG["head_dim"]
    + LINGLONG_CONFIG["waist_dim"]
)
...
def _read_position(f: h5py.File, group: str, expect_dim: int, num_frames: int) -> np.ndarray:
    """读取关节 position；若不存在或维度不一致则补零/截断"""
...
def load_linglong_h5(h5_path: Path) -> Dict[str, Any]:
    """
    从灵龙 LINGLONG-H H5 文件加载数据

    Returns:
        {
            'frames': int,
            'state':  np.ndarray (N, 24) = arm14 + effector4 + head2 + waist4
            'action': np.ndarray (N, 24)
    ...
    with h5py.File(h5_path, 'r') as f:
        # metadata（task_name 通常不在 metadata 里，靠 --task 命令行传）
        ...
        # ===== State / Action =====
        s_arm      = _read_position(f, 'joints/state/arm',      LINGLONG_CONFIG['arm_dim'],      num_frames)
        s_effector = _read_position(f, 'joints/state/effector', LINGLONG_CONFIG['effector_dim'], num_frames)
        s_head     = _read_position(f, 'joints/state/head',     LINGLONG_CONFIG['head_dim'],     num_frames)
        s_waist    = _read_position(f, 'joints/state/waist',    LINGLONG_CONFIG['waist_dim'],    num_frames)
        data['state'] = np.concatenate([s_arm, s_effector, s_head, s_waist], axis=1).astype(np.float32)

        a_arm      = _read_position(f, 'joints/action/arm',      LINGLONG_CONFIG['arm_dim'],      num_frames)
        a_effector = _read_position(f, 'joints/action/effector', LINGLONG_CONFIG['effector_dim'], num_frames)
        a_head     = _read_position(f, 'joints/action/head',     LINGLONG_CONFIG['head_dim'],     num_frames)
        a_waist    = _read_position(f, 'joints/action/waist',    LINGLONG_CONFIG['waist_dim'],    num_frames)
        data['action'] = np.concatenate([a_arm, a_effector, a_head, a_waist], axis=1).astype(np.float32)

        # ===== 相机图像 =====
        images: Dict[str, List[np.ndarray]] = {}
        image_shapes: Dict[str, tuple] = {}
        target_width, target_height = LINGLONG_CONFIG["target_image_size"]

        for cam_id in LINGLONG_CONFIG["cameras"]:
...
def convert_episode(
    ...
    try:
        ...
        dataset = LeRobotDataset.create(
            ...
            root=episode_dir,
            robot_type=LINGLONG_CONFIG["robot_type"],
            fps=fps,
            features=features,
            use_videos=True,
...
def convert_episode_wrapper(args: tuple) -> dict:
    ...
    try:
        episode_data = load_linglong_h5(h5_path)
...
def main():
    parser = argparse.ArgumentParser(
        description="灵龙 LINGLONG-H 机器人数据转换为 LeRobot v3.0 格式"
    )
    ...
    print(f"\nRobot: {LINGLONG_CONFIG['robot_type']}")
    print(f"State/Action dim: {STATE_DIM} (arm14 + effector4 + head2 + waist4)")
    print(f"Cameras: {LINGLONG_CONFIG['cameras']}")
