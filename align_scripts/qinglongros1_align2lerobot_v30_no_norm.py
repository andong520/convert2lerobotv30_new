...
数据结构:
    State/Action:
        ...
        - effector: 2维 (left_gripper + right_gripper), 保留原始值（不做归一化）
...
def load_qinglong_h5(h5_path: Path) -> Dict[str, Any]:
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        effector_state = np.clip(f['joints/state/effector/position'][:], 0, 90)  # (N, 2) clip到[0, 90]
        ...
        effector_action = np.clip(f['joints/action/effector/position'][:], 0, 90)  # (N, 2) clip到[0, 90]
