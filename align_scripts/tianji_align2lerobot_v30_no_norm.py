#!/usr/bin/env python3
"""
天机机器人 (TIANJI) 数据转换脚本 - 转换为 LeRobot 格式

数据结构:
    State/Action:
        - arm: 14维 (left_arm 7 + right_arm 7), 关节名 l_j1..l_j7, r_j1..r_j7
        - effector: 2维 (left_gripper + right_gripper), 保留原始值 (clip 到 [0, 100], 不做归一化)

    相机:
        - head: 720x1280 RGB -> resize 480x640
        - hand_left: 1080x1920 RGB -> resize 480x640
        - hand_right: 1080x1920 RGB -> resize 480x640

文件命名约定:
    {任务名}_{id}_align.h5  (如: 把药品放入药箱_s13d996b0d23492f9f8c29c297cb6840_align.h5)

Usage:
    python tianji_align2lerobot_v30.py \
        --input ./raw_data \
        --output ./lerobot_dataset \
        --repo_id tianji/task_name \
        --fps 30 --workers 4
"""
...
def extract_task_from_filename(filename: str, default_task: str = "manipulation_task") -> str:
    ...
    支持格式:
        把药品放入药箱_s13d996b0d23492f9f8c29c297cb6840_align.h5 -> 把药品放入药箱
        s1xxxxxx_align.h5 / s1xxxxxx.h5 -> 使用默认任务名
    """
    name = filename.replace('_align.h5', '').replace('.h5', '')
    parts = name.split('_')
...
def load_tianji_h5(h5_path: Path, task: str = None) -> Dict[str, Any]:
    """
    从天机机器人 H5 文件加载数据
    ...
    Returns:
        {
            'frames': int,
            'state': np.ndarray,  # (N, 16) = arm(14) + effector(2, 已归一化)
            'action': np.ndarray, # (N, 16) = arm(14) + effector(2, 已归一化)
            'images': {
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        effector_state = np.clip(f['joints/state/effector/position'][:], 0, 100)  # (N, 2) clip到[0, 100]
        data[state] = np.concatenate([
            arm_state[:, 0:7], effector_state[:, 0:1],    # 左臂7 + 左夹爪
            arm_state[:, 7:14], effector_state[:, 1:2],   # 右臂7 + 右夹爪
        ], axis=1).astype(np.float32)

        # Action: arm (14) + effector (2)
        arm_action = f['joints/action/arm/position'][:]  # (N, 14)
        effector_action = np.clip(f['joints/action/effector/position'][:], 0, 100)  # (N, 2) clip到[0, 100]
        data[action] = np.concatenate([
            arm_action[:, 0:7], effector_action[:, 0:1],
            arm_action[:, 7:14], effector_action[:, 1:2],
        ], axis=1).astype(np.float32)
...
def convert_episode(
    ...
    try:
        ...
        # 优先使用命令行指定的 task，否则使用 episode_data 中的 task
        ...
        for i in range(num_frames):
            ...
            # 添加占位符图像（实际视频走 _save_episode_video 通道）
            for cam_id, shape in episode_data['image_shapes'].items():
                h, w, c = shape
...
def convert_episode_wrapper(args: tuple) -> dict:
    ...
    try:
        episode_data = load_tianji_h5(h5_path, task=task_override)
...
def find_episodes(data_dir: Path) -> List[Path]:
    ...
    支持目录结构:
        1. data_dir/*_align.h5             (扁平: 多个 align 文件直接放在目录下)
        2. data_dir/序列号/*_align.h5      (子目录: 一个序列号一个文件夹)
        3. data_dir/序列号/*.h5            (子目录, 文件名不带 _align)
    """
    episodes = []
    seen = set()

    # 格式1: 直接在目录下的 *_align.h5 文件（优先匹配 _align）
    for h5_file in sorted(data_dir.glob("*_align.h5")):
        if h5_file not in seen:
            episodes.append(h5_file)
            seen.add(h5_file)

    # 格式2: 子目录中的 h5 文件 (优先选 *_align.h5)
    for subdir in sorted(data_dir.iterdir()):
        if subdir.is_dir():
            align_files = sorted(subdir.glob("*_align.h5"))
            if align_files:
                if align_files[0] not in seen:
                    episodes.append(align_files[0])
                    seen.add(align_files[0])
                continue
            # fallback: 子目录里没有 _align.h5 就找其他 .h5
            other_files = sorted([
                p for p in subdir.glob("*.h5")
                if not p.name.endswith('_normalize.h5')
            ])
            if other_files and other_files[0] not in seen:
                episodes.append(other_files[0])
                seen.add(other_files[0])

    # 格式3: 直接在目录下、不带 _align 后缀的 .h5（排除 _normalize.h5）
    for h5_file in sorted(data_dir.glob("*.h5")):
        if h5_file in seen:
            continue
        if h5_file.name.endswith('_normalize.h5'):
            continue
        episodes.append(h5_file)
        seen.add(h5_file)
...
def main():
    parser = argparse.ArgumentParser(
        description="天机机器人 (TIANJI) 数据转换为 LeRobot 格式"
    )
    ...
    parser.add_argument("--task", type=str, nargs='+', default=None,
                        help="任务描述（覆盖文件名提取出的任务名，多个词会自动拼接）")
    ...
    if args.task is not None and isinstance(args.task, list):
    ...
    # 为每个 episode 决定 task：命令行 --task 优先，否则按文件名提取
    episode_tasks = []
    for path in episodes:
        if args.task:
            episode_tasks.append(args.task)
        else:
            episode_tasks.append(extract_task_from_filename(path.name))

    print(f"Found {len(episodes)} episodes:")
    for path, t in zip(episodes, episode_tasks):
        print(f"  - {path.name}  (task: {t})")
    ...
    # 构建任务
    tasks = []
    for i, (path, t) in enumerate(zip(episodes, episode_tasks)):
        tasks.append((
            path, separate_dir, args.repo_id, i,
            args.fps, args.vcodec, args.crf, t
        ))
