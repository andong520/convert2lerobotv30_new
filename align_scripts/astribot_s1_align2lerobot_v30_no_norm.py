...
【no_norm 版本：所有维度保留原始物理量，effector 不做归一化，仅 clip 到 [0, 100] mm】
...
S1 机器人数据结构 (基于 metadata.json ver=2.1.0):
    State / Action: 25 维
        ...
        - effector:  2 维 (left_claw + right_claw)，单位 mm，范围约 [0, 100]
                     【本脚本保留原始 mm 值，不做归一化，但 clip 到 [0, 100]】
    ...
    备注：
        - h5 中还有 chassis(3) / state/robot(N,5,3 与 N,5,4) IMU/力矩数据，
          它们与 LeRobot 的 1D state vector 不兼容，chassis(3) 现已纳入（见 STATE_DIM），robot IMU 仍舍弃。

    相机 (默认):
        - head:       RGB (h5 原 720x1280)，统一 resize 到 480x640
        - hand_left:  RGB (h5 原 360x640)
        - hand_right: RGB (h5 原 360x640)
        H5 中还有 stereo 相机默认不导出；torso 已纳入默认导出（可 --cameras 调整）。
...
ASTRIBOT_S1_CONFIG = {
    ...
    "chassis_dim": 3,     # 底盘 x, y, yaw（joints/*/chassis/position，真实运动）
    # 默认导出 4 个相机（含 torso）；可通过 --cameras 覆盖（h5 中还有 stereo）
    "default_cameras": ["head", "hand_left", "hand_right", "torso"],
...
def load_astribot_s1_h5(h5_path: Path, cameras: List[str]) -> Dict[str, Any]:
    ...
    with h5py.File(h5_path, 'r') as f:
        ...
        s_effector = np.clip(s_effector, 0.0, 100.0)  # 夹爪开合度 mm，clip 到 [0, 100]
        ...
        data['state'] = np.concatenate([s_arm, s_effector, s_head, s_torso, s_chassis], axis=1).astype(np.float32)
        ...
        a_effector = np.clip(a_effector, 0.0, 100.0)  # 夹爪开合度 mm，clip 到 [0, 100]
        ...
        a_chassis  = _read_position(f, 'joints/action/chassis',  ASTRIBOT_S1_CONFIG['chassis_dim'],  num_frames)
        data['action'] = np.concatenate([a_arm, a_effector, a_head, a_torso, a_chassis], axis=1).astype(np.float32)
